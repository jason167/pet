...
如果采用PKCS#1格式，而不采用PKCS#8，对于java而言则实现如下：
...
开源的JCE实现包bcprov-jdk16-146.jar，下载地址：http://bouncycastle.org/latest_releases.html
</pre>
 *
 * @version $Id: RSAUtil_v3.java 2015年7月14日 下午1:57:31 $
 */
public class RSAUtil_v3 {
	...
	public static void main(String[] args){
		RSAUtil_v3 rsaEncrypt= new RSAUtil_v3();
		//rsaEncrypt.genKeyPair();

		//加载公钥
		try {
			rsaEncrypt.loadPublicKey(RSAUtil_v3.DEFAULT_PUBLIC_KEY);
		...
		//加载私钥
		try {
			rsaEncrypt.loadPrivateKey(RSAUtil_v3.DEFAULT_PRIVATE_KEY);
		...
		try {
			...
			System.out.println(RSAUtil_v3.byteArrayToString(cipher));
			System.out.println("明文长度:"+ plainText.length);
			System.out.println(RSAUtil_v3.byteArrayToString(plainText));
